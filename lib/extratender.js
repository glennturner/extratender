...
ExtraTender.prototype.addDiscussionEvents = function () {	
	...
	var discussionFormEle = document.getElementById( this.discussionFormId );
	
	for ( var i = 0, num = commentButtons.length; i < num; i++ ) {
		commentButtons[ i ].addEventListener( 'click', this.onButtonClick.bind( this ), true );
		discussionFormEle.addEventListener( 'click', this.onDiscussionFormEleClick.bind( this ), false );
	}	
};
...
ExtraTender.prototype.drawDiscussion = function ( ele, onSuccess, onError ) {
	...
	this.discussion.innerHTML = '<form name="discussion" onsubmit="return false;"><div id="extratender-text" class="original-body"></div><hr /><div id="extratender-form"><div><label id="extratender-reply-label" for="' + this.discussionTextareaId + '">Reply</label> \
