...
ExtraTender.prototype.drawAssets = function ( comment ) {
	...
	if ( comment.assets.length > 0 ) {
		...
		for ( var i = 0, len = comment.assets.length; i < len; i++ ) {
			commentStr += '<li class="extratender-asset asset"><a href="' + comment.html_href + comment.assets[ i ].download_path + '" rel="external">' + comment.assets[ i ].filename + '</a> (' + this.getAssetType( comment.assets[ i ].content_type ) + ' - ' + this.displayAssetSize( comment.assets[ i ].size ) + ')</li>';
		}
...
ExtraTender.prototype.displayAssetSize = function ( bytes ) {
	var fractional = 0,
		label = 'Byte';
