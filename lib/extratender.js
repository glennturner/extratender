...
ExtraTender.prototype.getActiveDiscussionId = function () {
	...
	if ( !discussion_id ) {	
		var parentEle = document.activeElement.parentNode;
		while ( !discussion_id && parentEle) {			
			discussion_id = this.getDiscussionIdFromElement( parentEle );
			...
			if ( !parentEle.parentNode ) {
				...
				parentEle = this.focusOnCommentTextarea() || this.focusOnFirstComment();
			}			
		}
	}
