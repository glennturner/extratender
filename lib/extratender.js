...
ExtraTender.prototype.addCategoryHTML = function ( discussion ) {	
	...
	this.catSelectEle.removeEventListener( 'change', this.addCategoryEvents.bind( this, did, cid ) );
...
ExtraTender.prototype.drawUserDiscussionCount = function ( d ) {
	...
	var countHTML = '<a href="#user-discussions" id="' + this.userDiscussionCountEle + '">' + d.discussions.length + ' Discussions</a>';
	
	var discussionListHTML = '<ul id="' + this.userDiscussionEle + '" class="extratender-discussion-list hide">';
	var date, disc, formatted_date;
	for ( var i = 0, num = d.discussions.length; i < num; i++ ) {
		...
		discussionListHTML += '<li><a href="' + disc.html_href + '">' + disc.title + '</a><span>' + disc.state + '</span><span>' + disc.comments_count + ' reply</span><span>' + formatted_date + '</span></li>';
	}
	discussionListHTML += '</ul>';
	
	// insert user count
	var actionBarEle = document.getElementById( this.discussionActionEle );
	actionBarEle.innerHTML = countHTML + actionBarEle.innerHTML;
	actionBarEle.innerHTML += discussionListHTML;
...
ExtraTender.prototype.removeDiscussionEvents = function () {
	...
	this.userSelectEle.addEventListener( 'change', this.assignUserToDiscussion.bind( this ) );
	document.getElementById( this.userDiscussionCountEle ).removeEventListener( 'click', this.toggleUserDiscussions.bind( this ) );
