...
ExtraTender.prototype.addCategoryEvents = function ( did ) {	
	// get categories
	this.tender.getCategories( (function ( req ) {
		...
		if ( cats.length > 0 && catSelectEle ) {
			for ( var i = 0, len = cats.length; i < len; i++) {
				catSelectEle.innerHTML += '<option value="' + this.getIdFromHref( cats[ i ].href ) + '">' + cats[ i ].name + '</option>';
			}
			...
			catSelectEle.addEventListener( 'change', 
				(function ( ele ) {
					...
					if ( cat_id ) {
						this.tender.changeCategoryOfDiscussion( cat_id, did, this.onRequestComplete.bind( this ), this.onRequestComplete.bind( this ));
					}
				}).bind( this )
			);
		}
...
ExtraTender.prototype.addUserEvents = function ( did ) {	
	// get users
	this.tender.getUsers( (function ( req ) {
		var users = req.users;
		console.log( 'USERS: ' );
		console.log( users );
		...
		if ( users.length > 0 && userSelectEle ) {
			...
			for ( var i = 0, len = users.length; i < len; i++) {
				userSelectEle.innerHTML += '<option value="' + this.getIdFromHref( users[ i ].href ) + '">' + users[ i ].name + '</option>';
			}
			...
			userSelectEle.addEventListener( 'change', 
				(function ( ele ) {
					...
					if ( user_id ) {
						this.tender.changeUserOfDiscussion( user_id, did, this.onRequestComplete.bind( this ), this.onRequestComplete.bind( this ));
					}
				}).bind( this )
			);
		}
...
ExtraTender.prototype.drawDiscussion = function ( ele, onSuccess, onError ) {
	...
	this.addCategoryEvents( discussionId );
	
	// no known way to assign a discussion to a user via the API documentation
	//this.addUserEvents( discussionId );	
