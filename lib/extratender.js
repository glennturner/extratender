...
ExtraTender.prototype.addSupportUserDiscussionCount = function ( discussion ) {
	...
	if ( discussion.author_email ) {
		filters.email = discussion.author_email;
	}
	...
	this.tender.getDiscussion( discussionId, 
		(function ( json ) {
			...
			if ( commentsNum >= 1 ) {				
				...
				var len = json.comments.length - 1;
				for ( var i = len; i >= 0 ; i-- ) {
					// don't display system messages
					if ( json.comments[ i ].system_message ) { continue; } 
					
					comment = json.comments[i];
					body += '<li class="extratender-comment"><p class="meta"><span class="number">' + i + '</span> Posted by ' + comment.author_name + ' on ' + this.displayDate( comment.created_at ) + '</p>';
