var ExtraTender = function ( account ) {
	...
	this.categoryLoaderId = 'cats-ajaxloader';
...
ExtraTender.prototype.addCategoryHTML = function ( discussion ) {
	...
	// get categories from server
		...
		this.tender.getCategories( (function ( req ) {
			...
			this.hideCategoryLoader();
			this.catSelectEle.disabled = true;
			this.catSelectEle.innerHTML += '<option value="" selected="selected">Cannot retrieve categories</option>';
		}).bind( this ));
	}
};

ExtraTender.prototype.showCategoryLoader = function () {
	var catLoader = document.getElementById( this.categoryLoaderId );
	catLoader.style.display = 'inline-block';
};

ExtraTender.prototype.hideCategoryLoader = function () {
	var catLoader = document.getElementById( this.categoryLoaderId );
...
ExtraTender.prototype.onDiscussionClick = function ( e ) {
	...
	// really hacky click-prevention for inputs/body copying/links
	if ( ele && e.srcElement.className != 'checkbox' && e.srcElement.className != 'original-body' && e.srcElement.tagName != 'a' ) {
...
ExtraTender.prototype.getActiveDiscussionElement = function () {
	...
	if ( active && !this.isDiscussion( active ) ) {
...
ExtraTender.prototype.enableCommentTextarea = function () {
	var ele = document.getElementById( this.discussionTextareaId );

	ele.className = ele.className.replace( ' disabled', '' );
	ele.removeAttribute( 'disabled' );
};

ExtraTender.prototype.disableCommentTextarea = function () {
	var ele = document.getElementById( this.discussionTextareaId );
...
ExtraTender.prototype.drawDiscussion = function ( ele, onSuccess, onError ) {
	...
	this.tender.getDiscussion( discussionId,
		(function ( json ) {
			...
			var commentsNum = json.comments ? ( json.comments.length - 1 ) : 0;
			var body = '';
			var attachments = [];
			...
			if ( commentsNum >= 1 ) {
				...
				for ( var i = commentsNum; i >= 0; i-- ) {
					...
					body += '<li class="extratender-comment' + ( comment.internal ? ' internal' : '' ) + '">';
					...
					//body += '<textarea readonly="readonly" class="original-body">' + comment.body + '</textarea>';
		...
		// an error occurred
		(function ( err ) {
			if ( err.status && confirm( 'The discussion could not be retrieved (' + err.status + '). Hide this discussion from being displayed?') ) {
...
ExtraTender.prototype.drawAssets = function ( comment ) {
	...
	if ( comment.assets.length > 0 ) {
		...
		for ( var i = 0, len = comment.assets.length; i < len; i++ ) {
			commentStr += '<li class="extratender-asset asset"><a href="' + comment.assets[ i ].download_path + '" rel="external">' + comment.assets[ i ].filename + '</a> (' + this.getAssetType( comment.assets[ i ].content_type ) + ' - ' + this.drawAssetSize( comment.assets[ i ].size ) + ')</li>';
		}
...
ExtraTender.prototype.toggleDiscussion = function ( srcEle ) {
  // ignore toggle if we're already viewing the discussion
  if ( this.discussionId == srcEle.getAttribute('discussion_id')) {
    return false;
  }

	var ele;
	if ( ele = this.ifDiscussion() ) {
		this.removeDiscussion();
	}


	if ( !ele || ( ele != this.discussion ) ) {
